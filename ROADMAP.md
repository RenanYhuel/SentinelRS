# Roadmap technique détaillée — SentinelRS (découpage précis, sans durée)
...
- Sous‑tâches :
  1.1. Vérifier et valider `Cargo.toml` workspace (member listing). Critère : `cargo metadata` passe.
  1.2. Ajouter `Makefile` ou `xtasks` pour builds courants (`make build-all`, `make fmt`, `make proto`). Critère : commandes exécutables.
  1.3. Créer `deploy/docker-compose.yml` minimal avec NATS+Postgres+Timescale pour dev E2E. Critère : `docker-compose up` lance les services.
- Dépendances : aucune.
...
- Sous‑tâches :
  2.1. Finaliser `proto/sentinel.proto` (metrics, batch, register, heartbeat, config messages). Critère : `protoc`/`prost_build` génère Rust sans warnings.
  2.2. Ajouter `build.rs` dans `crates/common` pour générer prost bindings automatiquement à la compilation. Critère : `cargo build` génère `common::proto` module.
  2.3. Implementer helpers Rust dans `crates/common/src` : canonicalize function pour Batch (deterministic bytes), batch_id generator, seq helper, Metric <-> JSON helpers. Critère : unit tests de roundtrip.
  2.4. Ajouter docs (README dans `crates/common`) expliquant versioning des proto (compat rules).
- Dépendances : 1.1, 1.2 (outil de build disponible).

Notes d'implémentation

- Prost avec `bytes = true`, `out_dir` dans `build.rs`.
- Canonicalization : utiliser prost bytes order (prost encodes deterministically for same message layout) et trier les maps (labels) explicitement avant sérialisation pour signature.
...
- 3.A.1. Implémenter module `collector::system` : fonctions pour lire CPU per-core, mem, disk, net, uptime, process count.
...
- 3.A.2. Normalisation des noms de métriques et labels (naming conventions). Documenter.
- 3.A.3. Abstraction `Collector` trait : `fn collect(&self) -> Vec<Metric>` pour permettre plugins et tests.
- Dépendances : 2.1 (types Metric/Batches).

    3.B Scheduler

- 3.B.1. Implementer scheduler simple (tokio):: spawn recurring tasks using `tokio::time::interval`.
- 3.B.2. Support per-check intervals and jitter config.
- Tests : simulate interval firing and ensure Collector invoked.

    3.C Buffer (WAL) API (interface only)

- 3.C.1. Exposer API `Wal::append(bytes) -> record_id`, `Wal::pop_ack(record_id)`, `Wal::iter_unacked()`.
- 3.C.2. WAL must be crash-safe: append then fsync policy configurable.
- Tests : append/read roundtrip using tempdir.
- Dépendances : 3.A, 3.B.

    3.D Batch composer

- 3.D.1. Compose batch from collected metrics and WAL records ordering.
- 3.D.2. Assign `batch_id`, `seq_start/seq_end` using monotonic counters persisted in metadata file.
- Tests : batch generation reproducible.

    3.E Signer & Compression

- 3.E.1. Implement HMAC signer util using `hmac + sha2`. API: `sign_batch(secret, bytes) -> signature`.
- 3.E.2. Compression wrapper: gzip compress when threshold.
- Tests : verify signatures and compressed flag.

    3.F Exporter gRPC client (tonic)

- 3.F.1. Stub client to call `AgentService::PushMetrics` using prost generated types. Metadata injection for `x-agent-id`, `x-signature`.
- 3.F.2. Implement send loop: take next batch, send, handle response codes (OK/RETRY/REJECTED).
- Tests : use mock server to verify metadata present and backoff logic.
- Dépendances : 2.1, 3.C, 3.D, 3.E.

    3.G Local HTTP (Prometheus exporter + health)

- 3.G.1. Run tiny HTTP server exposing `/metrics`, `/healthz`, `/ready` using `axum` or `hyper`.
- 3.G.2. Map internal agent metrics (queue length, wal size, last_send) to Prometheus.
- Tests : HTTP smoke test.

    3.H Config & secure store

- 3.H.1. Load `config.yml` via `serde_yaml`, validate schema.
- 3.H.2. Implement secure storage of `secret` via OS keystore abstraction with fallback AES‑GCM file encryption.
- Tests : config parsing + simple key store mock.

    3.I Integration test (agent minimal)

- 3.I.1. Run agent against local stub server (server stub that accepts PushMetrics and returns ACK). Validate WAL→send→ack→compaction.
- Acceptance : agent sends a batch and server receives valid batch with correct metadata.

---

4. Plugins WASM (wasmtime)

- But : permettre checks dynamiques safely.
- 4.1. Define plugin manifest format (YAML/JSON) fields: name, version, entry_fn, capabilities, resource_limits.
- 4.2. Implement plugin installer: download signed WASM blob, verify signature using server public key, store in `plugins_dir`.
- 4.3. Implement runtime loader using `wasmtime` API: instantiate module, provide host functions (http_get, read_file_limited, metric_builder API).
- 4.4. Resource constraints: set memory limit, CPU timeouts (wall clock), catch traps and report failure without crashing agent.
- 4.5. Define plugin API contract (Rust interface documented): call returns list of Metrics (serialized as JSON or via a shared memory buffer), include error handling.
- Tests : run sample plugins (nginx stub_status parser) in unit/integration.
- Dépendances : 3.A, 3.B, 3.G.

Notes

- Prefer passing minimal context (env, labels) to plugin to reduce host surface.

---

5. WAL (append-only) détaillé

- Format : segments `wal-000000N.log`. Record layout: [u32 len][protobuf bytes][u32 crc32].
...
- But : recevoir et valider batchs, publisher to NATS JetStream, expose REST endpoints for UI and admin.
...
- 7.3. Implement `PushMetrics` RPC handler:
    - Validate TLS
    - Validate `x-signature` HMAC using stored agent secret
    - Check timestamp within allowed window
    - Check `batch_id` for duplicate (idempotency cache)
    - Publish envelope to NATS JetStream subject `sentinel.metrics`
    - Respond quickly with status OK/RETRY/REJECTED
- 7.4. Expose REST endpoints for dashboard (see previous list): agents list, agent detail, metrics query proxy, alerts list, raw batch inspect.
...
- Use Tower middleware for auth and metrics. Keep handlers minimal and fast.
- Store idempotency cache in Redis or Postgres table with TTL.
...
- Start with single stream; expose scripts `deploy/nats-setup.sh` to create stream & consumers.

---

9. Workers (ingestion → storage → alerting)
   But : workers are le coeur du traitement lourd. Découper finement.
...
- Tests : integration tests with Timescale dev container.
    ...
    9.D Aggregator & rollups (worker responsibility)
...
- Use TimescaleDB native functions for downsampling & continuous aggregates where appropriate.
...
16. Observability & monitoring interne
...
- 17.2. `common/build.rs: prost codegen` — depends: 17.1.
...
- 17.13. `e2e: docker-compose for NATS+Postgres` — depends: 1.3.
...
- Platform specifics : agent compilation must support linux/windows/macos; but develop on Linux container first.
- Safety : design for at‑least‑once delivery and idempotence via `batch_id` server-side dedup.
